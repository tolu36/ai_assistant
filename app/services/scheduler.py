...
from app.services.calendar import list_busy_slots
from config import (
...
def _within_working_hours(start: datetime, duration_minutes: int) -> bool:
    ...
    day_start = start.replace(
        hour=SCHEDULER_DAY_START_HOUR,
        minute=0,
        second=0,
        microsecond=0,
    )
    day_end = start.replace(
        hour=SCHEDULER_DAY_END_HOUR,
        minute=0,
        second=0,
        microsecond=0,
    )
...
def _violates_constraints(start: datetime, duration_minutes: int) -> bool:
    ...
    for constraint in schedule_store.list_constraints():
        ...
        blocked_start = start.replace(
            hour=int(start_hour),
            minute=0,
            second=0,
            microsecond=0,
        )
        blocked_end = start.replace(
            hour=int(end_hour),
            minute=0,
            second=0,
            microsecond=0,
...
def propose_slots(
    ...
    requested_start = task_data.get("start_datetime") or _next_schedulable_slot(
        datetime.now(),
        duration,
    )
